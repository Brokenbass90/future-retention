...
## 🆕 Сессия 2026-06-25 — AI-локали, карандаш Pug-aware, конструктор, критич. фиксы

Всё ниже — локальные правки (на Heroku ничего не пушилось). Логика протестирована
offline (`npm test`), браузерные части — по `docs/CODEX-TEST-PLAN.md`.

### AI / локали
- Авто-маршрутизация: запросы-правки сами идут в tool-use агента (без тумблера 🤖),
  если есть ключ; zero-AI placeholderize — фолбэк offline.
- Vision: прикреплённые/перетащенные в чат картинки уходят агенту (`input_image`).
- `compare_locales` — сверка всех локалей с reference (отчёт).
- `align_locales_to_reference` — ДЕЙСТВИЕ: приводит все локали к структуре reference
  (число блоков/порядок, `{{embedded.*}}` на местах) → плейсхолдеры ложатся ровно.
- `insert_block` / `remove_block` — добавить/убрать блок в открытом HTML по якорю.
- См. `docs/AI-LOCALE-EDITING-UPGRADE.md`.

### Карандаш (инспектор превью)
- **Pug-aware**: на письмах из базы (Pug) правка идёт по токену `${{ ns.block_NN }}$` —
  стиль внедряется как `(style=…)` в строку Pug, текст уходит в активную локаль.
  На внешних HTML — как раньше (правка HTML).
- Ввод цвета хексом (`#ffffff`) + пипетка; фоновая картинка (URL) → полный набор
  (`background-image/position/repeat/size: cover`); слияние стилей по свойствам (без задвоения).
- Навигация по глубине ⬆/⬇ (выбор родителя) — собирается цепочка предков и в клике
  карандаша, и в значке ✎; контролы всегда видны.

### Конструктор `/constructor`
- Шаг 1: убрана абстрактная «Структура», 3 колонки (каталог | живое письмо | инспектор),
  перенос/удаление блока — в инспекторе. Drop блоков в превью уже работал.

### Workbench UX
- Разворот превью ⛶ (прячет колонку кода, ESC — назад).
- Меню «⋯» в тулбаре (Monaco, Outline, палитра блоков — через прокси-клики).
- Футер `footer_upload`: приглушён, последним, НЕ выгружается в экспорт; экспорт TXT — папкой.

### 🔴 Критические фиксы
- **«Сохранить» затирал исходник.** `addEventListener('click', saveCurrentSourceFile)`
  передавал объект события в `contentOverride` → в файл писалось `[object PointerEvent]`
  (так был убит `header.pug`/`header.jade` у mail-multiple-account-termination — восстановлены).
  Фикс: обработчик не передаёт событие + жёсткая проверка `typeof contentOverride === 'string'`.
- **Doctype при сборке** превращался в битый `<!DOCTYPE public>` (инлайнер ломал
  `doctype PUBLIC "…"`). Фикс в `email-base/tools/build-mail.js` (пост-инлайн нормализация
  doctype → корректный XHTML 1.0 Strict). Проверено сборкой.
- Карандаш на Pug больше не выдаёт «не найден фрагмент»; preview-артефакты (span-обёртки,
  debug-скрипты) чистятся перед сохранением.

### Тесты
- `npm test` — offline-логика (align/cross-check/html-blocks/agent-loop/locale-crud), всё зелёное.
- `npm run test:blocks` — компиляция блоков (19/23, 4 corrupted — известно).
- `docs/CODEX-TEST-PLAN.md` — план для Codex (автомат + браузерный чек-лист).

### Бэкапы правок сессии
`*.bak-ai*` рядом с `server.js`, `public/workbench.{js,css,html}`, `public/constructor.*`,
`src/ai-*.js`, `email-base/tools/build-mail.js`.

---

## 🆕 Сессия 2026-06-25 (продолжение) — карандаш, RTL, DnD, критфиксы. Build 2026-06-25h

Версию запущенного бандла видно в консоли: `[RetKit] build 2026-06-25h…` (`RETKIT_BUILD` в workbench.js).

### Карандаш (инспектор превью) — доведён
- **Pug-aware**: на письмах из базы стиль → `(style=…)` в строку Pug по токену `${{ ns.block_NN }}$`; текст → активная локаль; на внешнем HTML — правка HTML.
- Поля: текст, цвет (хекс+пипетка), фон (хекс; **прозрачный показывается пустым, не #000000**), **фоновая картинка (URL — подхватывается существующая)**, размер шрифта, **высота строки**, жирность, выравнивание, паддинг, border-radius, **граница (border)**, **ссылка (href) для `<a>`** (правится и на HTML, и на Pug).
- **Навигация по глубине ⬆/⬇** — цепочка предков из обработчика карандаша; счётчик `<tag> i/n`; всегда видна.
- **Box-model подсветка**: паддинг зелёный / маргин красный / бордер синий (при клике и смене слоя).
- 🔴 **Фикс**: контейнеры (`<td>`/`<div>` с детьми) больше НЕ «сплющиваются» при правке (раньше внутренний HTML заменялся плоским текстом → письмо ломалось). Стиль крупных блоков патчится через короткий открывающий тег (без обрезки outerHtml на 12000).

### RTL
- Кнопка **«⇄ RTL»** в тулбаре превью — приводит письмо к RTL по клику (идемпотентно, с откатом), без новой локали.
- `applyRtl` зеркалит стороны, центр оставляет центром, inline иконка+текст разворачивает (`direction:rtl`).
- ⏳ осталось: разворот двухъячеечных таблиц `img|text` (нужно миррорить в server `email-base/tools/rtl.js`).

### Конструктор `/constructor` (#2 DnD — в работе)
- 3 колонки (каталог | живое письмо | инспектор), «Структура» убрана.
- Drop блоков из каталога в превью (синяя линия вставки) — было.
- **Перетаскивание размещённых блоков для перестановки** — добавлено (`_draggingCanvasUid` + `moveCanvasUid`).
- ⏳ осталось: внутренние (inline) блоки внутрь секций со смарт-зонами; undo на N ходов (стек на 20 уже есть).

### Библиотека блоков (#1 — ✅ фундамент)
- 14 канонических блоков в `data/block-library/canonical/`, `npm run test:blocks` = **25/25, 0 битых**. 4 битых legacy-сниппета удалены.

### Workbench UX
- Monaco убран; в обычном редакторе **автозакрытие тегов/скобок**. Меню «⋯» (Outline, палитра).
- Разворот превью ⛶ (прячет колонку кода — `#editorPane`). Нижняя планка тянется почти на весь экран; «База писем» заполняет высоту (был фикс `eb-manager` 260px→100%).
- Футер `footer_upload` приглушён/последний/не в экспорте; экспорт TXT — папкой.

### 🔴 Критические фиксы (обязательно проверить после пуша)
- «Сохранить» затирал исходник объектом события (`[object PointerEvent]`) — починено (обработчик не передаёт событие + `typeof===string`). `header.pug/jade` восстановлены.
- Паддинг/стиль на крупном блоке ломал письмо — починено (см. карандаш).
- doctype `<!DOCTYPE public>` → корректный XHTML (`build-mail.js` пост-инлайн нормализация).

### Тесты и доки
- `npm test` — 6 наборов offline (+`test-html-validate`), зелёные.
- `docs/CODEX-TEST-PLAN.md` — полный актуальный чек-лист (build 2026-06-25h).
- `docs/STUDIO-ROADMAP.md` — план #1–#4 + карандаш-доработки + статус.
- `docs/CONSTRUCTOR-VISION.md`, `docs/AI-LOCALE-EDITING-UPGRADE.md`.

### Что осталось по приоритетам
1. ✅ Блоки из базы (фундамент готов; при желании — ещё узкие паттерны).
2. 🚧 DnD: внутренние блоки + undo (перестановка уже есть).
3. AI: compose/insert/remove — отполировать, прогнать вживую.
4. Figma/скриншот → блоки (нужен Figma-токен).
+ Карандаш: адаптивный размер тайтла (media-запросы); RTL: разворот таблиц img|text.

### Бэкапы правок: `*.bak-ai*` рядом с server.js, public/workbench.{js,css,html}, public/constructor.js, src/ai-*.js, email-base/tools/build-mail.js.
